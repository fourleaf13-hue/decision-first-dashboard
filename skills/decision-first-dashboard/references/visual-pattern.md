...
The spoke-style composition above is only the fallback for ordinary no-score inputs that contain heterogeneous raw KPIs. It is **not** a radar chart.

### No-score radar eligibility

A no-score dashboard may use the same closed radar visual family as composite without inventing an overall score, but only when all 3–6 dimensions already have source-backed comparable scores on one shared source-backed scale.
...
- A no-score radar additionally requires source grounding for the shared scale and every normalized vertex score.
...
For either an eligible no-score radar or a composite radar, 3–6 comparable dimensions form a true closed polygon:
...
Each polygon vertex is computed from the corresponding normalized score. A score change must move that vertex; spokes alone are not a radar chart.
