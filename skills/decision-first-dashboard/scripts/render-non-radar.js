...
.decision-layout{display:grid;gap:24px;align-items:stretch;min-height:610px}.decision-layout--center-only{grid-template-columns:minmax(0,900px);justify-content:center}.decision-layout--with-left{grid-template-columns:290px minmax(0,860px);justify-content:center}.decision-layout--with-right{grid-template-columns:minmax(0,860px) 300px;justify-content:center}.decision-layout--with-both{grid-template-columns:280px minmax(0,720px) 290px}
...
.synthesis-card{position:relative;min-height:610px;display:flex;align-items:center;justify-content:center}.synthesis-card:before{content:"";position:absolute;width:min(720px,92%);aspect-ratio:1;left:50%;top:50%;transform:translate(-50%,-50%);border-radius:50%;background:radial-gradient(circle,rgba(217,210,241,.34),rgba(238,241,250,.15) 62%,rgba(238,241,250,0) 78%);filter:blur(10px);pointer-events:none}
...
@media(max-width:620px){.dashboard-shell{width:calc(100vw - 24px);padding-top:28px}.synthesis-card{min-height:540px}.signal-lead{min-height:210px;padding:34px 24px}.signal-lead strong{font-size:48px}.signal-support-grid{grid-template-columns:1fr 1fr}}
`;
...
<body><main class="dashboard-shell">
  ...
  <section class="decision-layout ${layout}">
    ${left}
    <section class="synthesis-card">${htmlSignalFocus(data.signals)}</section>
    ${right}
  </section>
</main></body></html>`;
}
...
export function renderNonRadarSvg(data) {
  ...
  <style>.sans{font-family:Inter,ui-sans-serif,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.ink{fill:#24243a}.muted{fill:#747a9b}.accent{fill:#5448df}.positive{fill:#238764}.negative{fill:#d75b67}.neutral{fill:#747a9b}.support-panel,.lead-card,.support-signal-card{fill:#fff;stroke:#ececf5;stroke-width:1;filter:url(#panelShadow)}.lead-card{fill-opacity:.97}.support-signal-card{fill-opacity:.94}</style>
  <rect width="1440" height="900" fill="url(#pageBg)"/><circle cx="720" cy="450" r="390" fill="#e9e5f7" fill-opacity=".30"/>
  <g class="sans"><text x="72" y="76" class="ink" font-size="29" font-weight="760">Current overview</text><text x="72" y="107" class="muted" font-size="14">Key signals and supporting context</text>${left}${svgSignalFocus(data.signals)}${right}</g>
</svg>`;
}
