...
test('ambiguous hash streams still require identical file inventories and fingerprints', () => { /* assert mismatch */ });
test('contradictory, inherited, or error-marked acceptance results fail closed', () => { /* assert failure */ });
test('hostile thrown values still receive postflight checks', () => { /* assert postflight */ });
